...
bool socket_debug = true;

#define socket_info(fmt, arg...)    \
{                                   \
	if(socket_debug)                \
		log_info((fmt), ##arg);     \
}
...
int tcp_send_pkt(struct rt_net_client *client, const uint8_t *buf, uint16_t len)
{
	...
	socket_info("client:%p tcp send to ip:%s port:%d", client,
...
int udp_send_pkt(struct rt_net_client *client, const uint8_t *buf, uint16_t len)
{
	...
	socket_info("client:%p upd send to ip:%s port:%d", client,
		inet_ntoa(client->udp_opposite_addr.sin_addr),
		ntohs(client->udp_opposite_addr.sin_port));

	ret = sendto(client->udp_fd, (const void *)&client->udp_send_buf,
		ALIGN4(len) + sizeof(client->udp_send_buf.head),
		0,
		(struct sockaddr*)&client->udp_opposite_addr,
		sizeof(client->udp_opposite_addr));
...
static void *_tcp_recv_process(void *arg)
{
	...
	socket_info("tcp start recv from ip:%s port:%d", 
		inet_ntoa(client->tcp_client_addr.sin_addr),
		ntohs(client->tcp_client_addr.sin_port));
	...
	while(ret != -1)
	{
		...
		if(ret > 0)
		{
			_on_data_recv(client, buf->data, buf->head.data_len, NULL);
		}
	}

	socket_info("tcp stop recv from ip:%s port:%d", 
		inet_ntoa(client->tcp_client_addr.sin_addr),
		ntohs(client->tcp_client_addr.sin_port));
...
static void *_udp_recv_process(void *arg)
{
	...
	struct pkt_buf *buf = malloc(sizeof(struct pkt_buf));
	struct rt_net_client *client = (struct rt_net_client *)arg;
	uint32_t len = sizeof(client->udp_opposite_addr);

	if(client->udp_listen_addr.sin_port != 0)
	{
		socket_info("upd start recv from ip:%s port:%d",
			inet_ntoa(client->udp_listen_addr.sin_addr),
			ntohs(client->udp_listen_addr.sin_port));
	}

	while(ret > 0)
	{
		ret = recvfrom(client->udp_fd, (uint8_t *)buf, MAX_PKT_SIZE, 0, (struct sockaddr*)&client->udp_opposite_addr, &len);
		socket_info("client:%p upd recv from ip:%s port:%d", client,
			inet_ntoa(client->udp_opposite_addr.sin_addr),
			ntohs(client->udp_opposite_addr.sin_port));
	...
	if(client->udp_listen_addr.sin_port != 0)
	{
		socket_info("upd stop recv from ip:%s port:%d",
			inet_ntoa(client->udp_listen_addr.sin_addr),
			ntohs(client->udp_listen_addr.sin_port));
	}
...
int net_server_init(struct rt_net_server *server, char *addr, uint16_t port)
{
	...
	bzero(&server->tcp_listen_addr, sizeof(server->tcp_listen_addr));
	bzero(&server->udp_client.udp_listen_addr, sizeof(server->udp_client.udp_listen_addr));

	server->tcp_listen_addr.sin_family = AF_INET;
	server->tcp_listen_addr.sin_port = htons(port);
	if(addr)
	{
		server->tcp_listen_addr.sin_addr.s_addr = inet_addr(addr);
	}else{
		server->tcp_listen_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	}

	server->udp_client.udp_listen_addr.sin_family = AF_INET;
	server->udp_client.udp_listen_addr.sin_port = htons(port);
	if(addr)
	{
		server->udp_client.udp_listen_addr.sin_addr.s_addr = inet_addr(addr);
	}else{
		server->udp_client.udp_listen_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	}

	if(bind(server->tcp_listen_fd, (struct sockaddr *)&server->tcp_listen_addr, sizeof(server->tcp_listen_addr)) != 0)
	{
		func_error("tcp bind failed.");
		return -1;
	}

	if(bind(server->udp_client.udp_fd, (struct sockaddr *)&server->udp_client.udp_listen_addr, sizeof(server->udp_client.udp_listen_addr)) != 0)
	{
...
int net_client_init(struct rt_net_client *client, char *addr, uint16_t port)
{
	...
	bzero(&client->udp_opposite_addr, sizeof(client->udp_opposite_addr));
	bzero(&client->udp_listen_addr, sizeof(client->udp_listen_addr));
	...
	client->udp_opposite_addr.sin_family = AF_INET;
	client->udp_opposite_addr.sin_port = htons(port);
	client->udp_opposite_addr.sin_addr.s_addr = inet_addr(addr);


	client->udp_listen_addr.sin_family = AF_INET;
	client->udp_listen_addr.sin_port = htons(port + 1);
	if(addr)
	{
		client->udp_listen_addr.sin_addr.s_addr = inet_addr(addr);
	}else{
		client->udp_listen_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	}

	if(bind(client->udp_fd, (struct sockaddr *)&client->udp_listen_addr, sizeof(client->udp_listen_addr)) != 0)
	{
		func_error("udp bind failed.");
		return -1;
	}
