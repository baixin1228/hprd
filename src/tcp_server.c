...
char video_pkt[BUFLEN];
uint32_t video_pkt_len;
pthread_spinlock_t video_pkt_lock;
...
my_event_st *create_my_event(int fd, int events, void *callback_fn) {
	...
	my_events[my_events_len++] = (my_event_st) {
		.fd = fd,
		.events = events,
...
void check_active(int epfd, int n) {
	time_t now = time(0);
	for (int i = 1; i <= n; ++i) {
		if (i >= my_events_len)
			return;
...
void *bradcast_data(void *opaque)
{
	int epfd = *(int *)opaque;
	my_event_st *ev;
	while(1)
	{
		...
		if(video_pkt_len > 0)
		{
			...
			{
				ev = &my_events[i];
				memcpy(ev->buf, video_pkt, video_pkt_len);

				ev->callback_fn = (void (*)(int, void *))callback_senddata;
				ev->len = video_pkt_len;
				ev->events = EPOLLOUT;
				register_event(epfd, ev);
			}
			video_pkt_len = 0;
...
void *tcp_server_thread(void *opaque) {
	...
	pthread_create(&p1, NULL, bradcast_data, &epfd);
